...
| Capability | Twenty 2.0 | Fourty | Notes |
|---|---|---|---|
| Native MCP server (self-host) | ✅ Claude/ChatGPT/Cursor read+write [4][6] | ✅ stdio JSON-RPC + HTTP, 26 tools, RLS+RBAC+field permissions (Gate B6, ADR-010) | `npm run mcp`; verified end-to-end + `mcp.test.ts`, `mcp-neighbours.test.ts`. |
| AI agents / chat in-app | ✅ [4] | ✅ per-record agent panel + durable multi-conversation history, plus a background work ledger (`agent_tasks`, two lanes) that keeps records true with **no** API key; BYO OpenAI-compatible provider for the chat half, optional and off by default (ADR-015/016/018) | `ai-agent.test.ts`, `ai-chat-api.test.ts`, `ai-conversations.test.ts`, `agent-panel-*.test.ts`, `agent-tasks-*.test.ts`; writes RBAC/RLS-gated + audited `via:'ai'`/`via:'research'`. |
